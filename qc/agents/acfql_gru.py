...
class ACFQLAgent_GRU(flax.struct.PyTreeNode):
    ...
    def actor_loss(self, batch, grad_params, rng):
        ...
        # Total loss.
        actor_loss = bc_flow_loss + self.config['alpha'] * distill_loss + 0.01*q_loss
    ...
    def compute_flow_actions_grad(
        ...
        """Compute actions from the BC flow model using the Euler method."""
        if self.config['encoder'] is not None:
            observations = self.network.select('actor_bc_flow_encoder')(observations,params=grad_params)
        actions = noises
        # Euler method.
        for i in range(self.config['flow_steps']):
            t = jnp.full((*observations.shape[:-1], 1), i / self.config['flow_steps'])
...
def get_config():
    ...
    config = ml_collections.ConfigDict(
        dict(
            agent_name='acfql_gru',  # Agent name.
