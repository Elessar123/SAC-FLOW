...
class ACFQLAgent_GRUOffline(flax.struct.PyTreeNode):
    ...
    def actor_loss(self, batch, grad_params, rng):
        ...
        if self.config["actor_type"] == "distill-ddpg":
            ...
            target_flow_actions = self.compute_flow_actions_grad(batch['observations'], noises=noises,grad_params=grad_params)
            ...
            qs = self.network.select(f'critic')(batch['observations'], actions=target_flow_actions)
    ...
    def actor_loss_offline(self, batch, grad_params, rng):
        """Compute the FQL actor loss."""
        if self.config["action_chunking"]:
            batch_actions = jnp.reshape(batch["actions"], (batch["actions"].shape[0], -1))  # fold in horizon_length together with action_dim
        else:
    ...
    @jax.jit
    def total_loss_offline(self, batch, grad_params, rng=None):
        """Compute the total loss."""
        info = {}
        rng = rng if rng is not None else self.rng
        ...
        actor_loss, actor_info = self.actor_loss_offline(batch, grad_params, actor_rng)
        for k, v in actor_info.items():
            info[f'actor/{k}'] = v
    ...
    @staticmethod
    def _update_offline(agent, batch):
        """Update the agent and return a new agent with information dictionary."""
        new_rng, rng = jax.random.split(agent.rng)

        def loss_fn(grad_params):
            return agent.total_loss_offline(batch, grad_params, rng=rng)
    ...
    def compute_flow_actions_grad(
        ...
        for i in range(self.config['flow_steps']):
            t = jnp.full((*observations.shape[:-1], 1), i / self.config['flow_steps'])
            vels = self.network.select('actor_bc_flow')(observations, actions, t, is_encoded=True,params=grad_params)
            actions = actions + vels / self.config['flow_steps']
        actions = jnp.clip(actions, -1, 1)
        return actions
    ...
    def create(
        ...
        actor_bc_flow_def = ActorVectorFieldGRU(
...
def get_config():
    ...
    config = ml_collections.ConfigDict(
        dict(
            agent_name='acfql_gru_offline',  # Agent name.
