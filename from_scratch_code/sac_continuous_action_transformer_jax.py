...
class TransformerActor(nn.Module):
    ...
    def __call__(self, obs, key, training=True):
        """
        Transformer forward pass - similar to TD3 but with stochastic sampling for SAC
        Args:
        ...
        log_std_head = nn.Dense(self.action_dim, name='log_std_head')
        ...
        # Autoregressive generation for denoising_steps
        for step in range(self.denoising_steps):
            ...
            # Get mean and log_std for the next step (last position in sequence)
            mean_next = mean_head(output[:, -1:, :])  # [batch_size, 1, action_dim]
            log_std_next = log_std_head(output[:, -1:, :])  # [batch_size, 1, action_dim]
            log_std_next = jnp.tanh(log_std_next)
            log_std_next = self.log_std_min + 0.5 * (self.log_std_max - self.log_std_min) * (log_std_next + 1)
            std_next = jnp.exp(log_std_next)
            
            # Sample next action with noise injection (stochastic for SAC)
            key, sample_key = jax.random.split(key)
            noise = jax.random.normal(sample_key, mean_next.shape)
            x_next = mean_next + std_next * noise
            x_next = jnp.squeeze(x_next, 1)  # [batch_size, action_dim]
            
            # Calculate log probability for this step
            step_log_prob = jax.scipy.stats.norm.logpdf(noise.squeeze(1), 0, 1).sum(axis=-1, keepdims=True)
...
if __name__ == "__main__":
    args = tyro.cli(Args)
    run_name = f"{args.env_id}__{args.exp_name}__{args.seed}_transformer_steps{args.denoising_steps}__{int(time.time())}"
    if args.track:
        import wandb
    ...
    # Initialize actor with obs only (like TD3)
    ...
    def update_critic(
        ...
        # Sample next actions from current policy using transformer
