...
class Args:
    ...
    flow_variant: str = "no_tanh"
    """GRU flow variant: original, simplified, no_tanh"""
...
class Actor(nn.Module):
    ...
    def __call__(self, obs):
        ...
        for step in range(self.denoising_steps):
            ...
            # Time embedding
            time_emb = time_emb_net(t)
            
            # Concatenate obs, x, and time_emb
            flow_input = jnp.concatenate([obs, x_current, time_emb], axis=-1)
...
if __name__ == "__main__":
    args = tyro.cli(Args)
    run_name = f"{args.env_id}__{args.exp_name}__{args.seed}_steps{args.denoising_steps}_{args.flow_variant}__{int(time.time())}"
    if args.track:
        import wandb
