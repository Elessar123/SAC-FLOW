...
class Actor(nn.Module):
    ...
    def setup(self):
        ...
        # Add BatchRenorm layer for CrossQ style
        if self.use_batch_norm:
    ...
    def flow_step(self, obs, x_prev, t, training: bool):
        ...
        h = x_concat
        # Apply BatchRenorm if enabled
        if self.use_batch_norm:
            h = self.bn(h, use_running_average=not training)
...
if __name__ == "__main__":
    args = tyro.cli(Args)
    run_name = f"{args.env_id}__crossq_flow__{args.exp_name}__{args.seed}__steps{args.denoising_steps}__{int(time.time())}"
    if args.track:
        import wandb
