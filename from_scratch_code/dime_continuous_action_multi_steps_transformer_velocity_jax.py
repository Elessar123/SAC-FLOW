# CrossQ implementation with Transformer-based Flow Actor - With Drift and Ratio
import os
...
@dataclass
class Args:
    exp_name: str = "transformer-drift-ratio"
    ...
    prior_std: float = 1.0
    """prior standard deviation for Gaussian prior"""
    # wandb
    wandb_project_name: str = "sacflow-fromscratch-Walker2d-v4"
...
class TransformerFlowActor(nn.Module):
    """
    Transformer-based Flow Matching Actor with Drift and Ratio
    """
    action_dim: int
    ...
    def drift_fn(self, x):
        return -x / (self.prior_std ** 2)
    
    def prior_log_prob(self, x):
        return jnp.sum(
            -0.5 * (x / self.prior_std)**2 
    ...
    def __call__(self, obs, key, training: bool = True):
        ...
        Returns:
            ...
            forward_log_prob: simple forward log probability
            ratio_log_prob: log probability with forward-backward ratio correction
        """
        ...
        # Generate initial random action x0 ~ N(0, σ²I)
        key, init_key = jax.random.split(key)
        x_current = jax.random.normal(init_key, (batch_size, self.action_dim)) * self.prior_std
        
        forward_log_prob = self.prior_log_prob(x_current)
        ratio_log_prob = self.prior_log_prob(x_current)
        
        # Flow Matching iterative velocity-based refinement
        for step in range(self.denoising_steps):
            # === Forward step: drift + network velocity ===
            ...
            # Sample network velocity with noise injection
            key, sample_key = jax.random.split(key)
            velocity_noise = jax.random.normal(sample_key, network_velocity_mean.shape)
            network_velocity = network_velocity_mean + velocity_std * velocity_noise
            
            # Add drift to get total velocity
            drift = self.drift_fn(x_current)
            total_velocity_fwd = drift + network_velocity
            
            # Forward update: x_{t+1} = x_t + total_velocity * Δt
            fwd_mean = x_current + total_velocity_fwd * DELTA_T
            
            # Resample for the actual next state (using same std)
            key, sample_key = jax.random.split(key)
            noise = jax.random.normal(sample_key, fwd_mean.shape)
            x_new = fwd_mean + velocity_std * noise
            
            # Calculate forward log prob
            fwd_log_prob = self.log_prob_gaussian(x_new, fwd_mean, velocity_std)
            forward_log_prob += fwd_log_prob
            
            # === Backward step: only drift (no network) ===
            drift_bwd = self.drift_fn(x_new)
            bwd_mean = x_new + drift_bwd * DELTA_T
            
            # Backward log prob (using same std)
            bwd_log_prob = self.log_prob_gaussian(x_current, bwd_mean, velocity_std)
            log_ratio = fwd_log_prob - bwd_log_prob
            ratio_log_prob += log_ratio
        ...
        forward_log_prob -= tanh_correction
        ratio_log_prob -= tanh_correction
        
        return action, forward_log_prob, ratio_log_prob
...
if __name__ == "__main__":
    args = tyro.cli(Args)
    run_name = f"{args.env_id}__transformer_drift_ratio__{args.exp_name}__{args.seed}__steps{args.denoising_steps}__prior{args.prior_std}__{int(time.time())}"
    if args.track:
        import wandb
        ...
        wandb.init(
            ...
            config=vars(args),
            name=run_name,
            group="crossq_transformer_drift_ratio"
        )
    ...
    def update_critic(
        ...
        next_actions, next_forward_log_prob, _ = actor_apply_inference(
        ...
        def mse_loss(params, batch_stats):
            ...
            next_q_values = next_q_values - alpha_value * next_forward_log_prob
    ...
    def update_actor_and_alpha(
        ...
        def actor_loss_fn(actor_params, actor_batch_stats):
            (actions, _, ratio_log_prob), new_actor_batch_stats_dict = actor_apply_train(
            ...
            actor_loss = (alpha_value * ratio_log_prob - min_qf_pi).mean()
            new_batch_stats = new_actor_batch_stats_dict.get('batch_stats', {})
            return actor_loss, (ratio_log_prob.mean(), new_batch_stats)
    ...
    for global_step in range(args.total_timesteps):
        if global_step < args.learning_starts:
            ...
            key, sample_key = jax.random.split(key, 2)
            actions, _, _ = actor_apply_inference(
    ...
    if len(all_episode_returns) > 0:
        ...
        if args.track:
            wandb.log({
                ...
                "final_stats/prior_std": args.prior_std
