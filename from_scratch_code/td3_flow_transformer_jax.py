...
# Alternative implementation that builds action sequence (closer to original structure)
class FlowMatchingTransformerActorSequential(nn.Module):
    """
    Alternative Flow Matching implementation that builds full sequence
    This is closer to the original autoregressive structure but uses Flow Matching principles
    """
    action_dim: int
    ...
    def __call__(self, obs):
        ...
        for step in range(self.denoising_steps):
            ...
            predicted_velocity = velocity_head(output[:, -1:, :])  # [batch_size, 1, action_dim]
...
TransformerActor = FlowMatchingTransformerActorSequential
...
if __name__ == "__main__":
    args = tyro.cli(Args)
    run_name = f"{args.env_id}__{args.exp_name}__{args.seed}_flow_matching_steps{args.denoising_steps}_d{args.d_model}_h{args.n_head}_l{args.n_layers}__{int(time.time())}"
    if args.track:
        import wandb
