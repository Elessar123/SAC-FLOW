...
class Args:
    ...
    """number of denoising steps for flow-based actor"""
...
class Actor(nn.Module):
    ...
    def __call__(self, obs):
        ...
        # Generate initial random point x0 ~ N(0, I) - keep this randomness
        # Use a deterministic key based on obs for reproducibility during evaluation
        # but still maintain some initial randomness
        ...
        # Define network layers - these will be shared across all denoising steps
        fc1 = nn.Dense(256, name='fc1')
        fc2 = nn.Dense(256, name='fc2')
        fc_mean = nn.Dense(self.action_dim, name='fc_mean')
        
        # Denoising steps - deterministic version (no sampling noise)
        ...
        for step in range(self.denoising_steps):
            ...
            # Concatenate obs, previous x, and time
            x_input = jnp.concatenate([obs, x_current, t], -1)
            
            # Forward pass through network
            x_input = nn.relu(fc1(x_input))
            x_input = nn.relu(fc2(x_input))
            mean = fc_mean(x_input)
            
            # Deterministic update: use mean directly (no sampling)
            x_current = mean
        
        # Apply tanh transformation and scaling
        x_current = jnp.tanh(x_current)
        action = x_current * self.action_scale + self.action_bias
        return action
...
if __name__ == "__main__":
    args = tyro.cli(Args)
    run_name = f"{args.env_id}__{args.exp_name}__{args.seed}_steps{args.denoising_steps}__{int(time.time())}"
    if args.track:
        import wandb
    ...
    actor = Actor(
        ...
        denoising_steps=args.denoising_steps,  # Added denoising_steps parameter
    )
