...
class Actor(nn.Module):
    ...
    def setup(self):
        ...
        # Standard deviation network (based on observation only)
    ...
    def __call__(self, obs, key, training: bool = True):
        ...
        for step in range(self.denoising_steps):
            ...
            # Pass training flag to flow_step for BatchRenorm
...
if __name__ == "__main__":
    args = tyro.cli(Args)
    run_name = f"{args.env_id}__CrossQ_Flow_GRU__{args.seed}__{int(time.time())}"
    if args.track:
        import wandb
    ...
    # MODIFIED: Actor instantiation updated for the new flow-based actor
    actor = Actor(
