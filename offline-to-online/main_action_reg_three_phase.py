...
flags.DEFINE_float('online_alpha', 100.0, "Change the Alpha for online training")
...
def main(_):
    ...
    flow_step = FLAGS.agent['denoising_steps']
    exp_name = f"{agent_name}_{task_name}_{base_exp_name}_{flow_step}"
    ...
    ## Use different Alpha for online training
    agent = agent.replace(config=agent.config.copy({'alpha': FLAGS.online_alpha}))
    print(f"The alpha in online step: {agent.config['alpha']}")
